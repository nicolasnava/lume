...
interface BookingWizardModalProps {
  ...
  initialServico: ServicoRow | null
...
export default function BookingWizardModal({
  ...
  useEffect(() => {
    if (initialServico) {
      setSelectedServicos([initialServico])
    } else {
      setSelectedServicos([])
    }
    setStep(1)
  ...
  }, [step, isOpen, profissional.id, profissional.janela_agendamento_dias])
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-xs p-4 animate-in fade-in duration-200">
      <div className="relative w-full max-w-xl max-h-[90vh] overflow-y-auto rounded-3xl bg-white p-6 shadow-2xl space-y-6 border border-gray-100">
        {/* Header do Modal */}
        <div className="flex items-center justify-between border-b border-gray-100 pb-3">
          <div>
            ...
            <h2 className="text-base font-bold text-[#4A3F5C]">
        ...
        {bookingSuccess ? (
          <div className="py-6 text-center space-y-5">
            ...
            <button
              type="button"
              onClick={onClose}
              className="w-full py-3 rounded-xl font-semibold text-xs text-white shadow-md transition hover:opacity-90 cursor-pointer"
          ...
          <>
                  ...
                  {selectedServicos.length === 0 ? (
                    ...
                    <div className="space-y-2">
                      {selectedServicos.map((servico) => (
                        <div
                          key={servico.id}
                          className="flex items-center justify-between bg-purple-50/50 p-3 rounded-2xl border border-[#B8A9D9]/40 gap-3"
                        >
                          <div className="space-y-0.5 min-w-0 flex-1">
                            <h4 className="text-xs font-semibold text-[#4A3F5C] break-words line-clamp-2">
                              {servico.nome}
                            </h4>
                          ...
                          {/* Item 10: Ícone de lixeira sempre visível independentemente da quantidade de serviços */}
                          <button
                            type="button"
                  ...
                  <div className="space-y-2 pt-2 border-t border-gray-100">
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-2.5">
                      {servicosSugeridos.map((s) => (
                        <div
                          key={s.id}
                          className="flex flex-col justify-between bg-white p-2.5 rounded-2xl border border-gray-200 hover:border-[#B8A9D9] transition space-y-2 group shadow-2xs"
                        >
                          ...
                          <div className="space-y-0.5 flex-1 min-w-0">
                            <p className="text-xs font-bold text-[#4A3F5C] leading-snug line-clamp-2">
                              {s.nome}
                            </p>
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2">
                      <span className="text-xs font-semibold text-[#4A3F5C] flex items-center gap-1">
                        <Clock className="h-3.5 w-3.5 text-[#4A3F5C]/70" />
            ...
            {step === 2 && (
              <div className="space-y-4">
                ...
                <div className="space-y-1">
                  <h3 className="text-sm font-bold text-[#4A3F5C]">Escolha o dia do atendimento</h3>
                  <p className="text-xs text-gray-500 font-medium">
            ...
            {step === 4 && (
              <form onSubmit={handleSubmitBooking} className="space-y-4">
                ...
                {/* Card Resumo do Agendamento (Item 11: Textos importantes ampliados) */}
                <div className="rounded-3xl bg-[#FAF7F5] p-4 sm:p-5 border border-[#B8A9D9]/40 space-y-4 shadow-xs">
                  <div className="flex items-center gap-3 border-b border-gray-200/60 pb-3">
                    <div className="relative h-11 w-11 shrink-0 overflow-hidden rounded-full border border-gray-200 bg-white shadow-2xs">
                    ...
                    <div className="min-w-0">
                      <h4 className="text-base font-extrabold text-[#4A3F5C] truncate">{profissional.nome}</h4>
                      {profissional.tagline && (
                        <p className="text-xs text-gray-500 italic truncate">{profissional.tagline}</p>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="space-y-2 text-xs sm:text-sm font-medium">
                    <div className="flex items-start gap-2.5">
                      ...
                      <div className="flex-1">
                        ...
                        <strong className="text-sm font-extrabold text-[#4A3F5C] block leading-tight mt-0.5">
                          {selectedServicos.map((s) => s.nome).join(' + ')}
                        </strong>
                    ...
                    <div className="flex items-start gap-2.5 pt-2.5 border-t border-gray-200/60">
                      <Calendar className="h-4 w-4 text-[#B8A9D9] shrink-0 mt-0.5" />
                      <div className="flex-1">
                        <span className="text-xs text-gray-500 font-semibold block uppercase tracking-wider">Data e Hora:</span>
                        <strong className="text-sm font-extrabold text-[#4A3F5C] block capitalize mt-0.5">
                          {selectedDateStr &&
                            new Date(selectedDateStr + 'T00:00:00').toLocaleDateString('pt-BR', {
                              weekday: 'short',
                              day: '2-digit',
                ...
                {/* Banner Informativo */}
                <div className="rounded-2xl bg-purple-50 p-3.5 border border-[#B8A9D9]/40 flex items-start gap-2.5 text-xs text-[#4A3F5C]">
                  <Zap className="h-4 w-4 text-[#4A3F5C] shrink-0 mt-0.5" />
                  <p className="text-xs leading-relaxed font-medium">
                    <strong className="font-bold">Agendamento Automático Ativo</strong> — Seu horário será confirmado na hora! Você receberá o comprovante imediatamente.
                  </p>
                </div>
                ...
                <div className="space-y-3.5">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full rounded-2xl border border-gray-200 bg-gray-50 p-3 text-xs sm:text-sm text-[#4A3F5C] font-semibold focus:border-[#B8A9D9] focus:bg-white focus:outline-hidden"
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-500 font-medium mt-1">
                      A confirmação do agendamento e lembretes chegarão por este número.
                    </p>
                  </div>
                  ...
                  {/* Formas de Pagamento em Cards Quadrados Clicáveis */}
                      ...
                      {PAYMENT_OPTIONS.map((opt) => {
                        ...
                        return (
                          <button
                            ...
                            className={`flex flex-col items-center justify-center p-3 rounded-2xl border transition cursor-pointer gap-1.5 text-center ${
                              isSelected
                          ...
                          >
                            ...
                            >
                              <PaymentIcon method={opt.id} className="h-4 w-4" />
                            </div>
                ...
                <div className="pt-2">
                  <button
                    type="submit"
                    disabled={submitting}
                    className="w-full py-3.5 rounded-2xl font-extrabold text-sm text-white shadow-md transition hover:opacity-90 disabled:opacity-50 cursor-pointer flex items-center justify-center gap-2"
                    style={{ backgroundColor: corPrimaria, color: textColorOnPrimary }}
                  >
                    {submitting ? (
                      <>
